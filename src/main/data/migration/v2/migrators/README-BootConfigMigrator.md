...
Unlike other migrators, it writes to a **file-based store** (`~/.modauistudio/boot-config.json`) rather than a SQLite table. See [Boot Config Overview](../../../../../../docs/references/data/boot-config-overview.md) for why this system exists.
...
|---|---|---|---|
| `~/.modauistudio/config/config.json` → `appDataPath` | `app.user_data_path` | `Record<string, string>` | *(null — see below)* |
...
- AppImage: `path.dirname(APPIMAGE) + '/modaui-studio.appimage'`
- Windows portable: `PORTABLE_EXECUTABLE_DIR + '/modaui-studio-portable.exe'`

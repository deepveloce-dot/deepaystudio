...
import type { ModauiClawConfiguration, GetAgentSessionResponse } from '@types'
...
import { PromptBuilder } from '../modauiclaw/prompt'
...
class ClaudeCodeService implements AgentServiceInterface {
  ...
  async invoke(
    ...
    if (soulEnabled && cwd) {
      soulSystemPrompt = await promptBuilder.buildSystemPrompt(cwd, agentConfig as ModauiClawConfiguration | undefined)

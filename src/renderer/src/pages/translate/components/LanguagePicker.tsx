...
const LanguagePicker: FC<Props> = ({ value, onChange, disabled, className }) => {
  ...
  const options = useMemo(() => translateLanguages.filter((l) => l.langCode !== UNKNOWN.langCode), [translateLanguages])

  const selected = getLanguageByLangcode(value)
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-expanded={open}
          className={cn(
            'flex h-8 w-full items-center justify-between gap-2 rounded-3xs border border-border/40 bg-transparent px-2.5 text-xs transition-colors hover:bg-muted/30 disabled:cursor-not-allowed disabled:opacity-60',
      ...
      <PopoverContent
        align="start"
        sideOffset={4}
        className="w-(--radix-popover-trigger-width) rounded-xs border border-border bg-popover p-1 shadow-xl">
        <div
          role="listbox"
          ...
          {options.map((lang) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex w-full items-center gap-2 rounded-3xs px-2 py-[6px] text-left text-xs transition-colors',
                  isSelected

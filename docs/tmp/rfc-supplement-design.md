# RFC 补充设计：文件管理（单一节点表 + 文件树）

本文档基于 `rfc-file-manager.md` 的评审结论（见 `rfc-review.md`），补充细化了以下设计：

1. 双模式存储设计（托管 vs 外部）
2. Provider 扩展模型（含远程文件预留）
3. Drizzle Schema 定稿
4. 删除与恢复策略（OS 风格 Trash）
...
RFC 原文声称"OS 目录可直接查看"（第 72 行），又要求"物理文件名为 `id + ext`"（第 129 行）。两者不兼容，且笔记系统必须使用人类可读文件名以支持外部编辑器。

### 1.2 方案：Provider 驱动的挂载点
...
- **`remote`**：本地缓存路径（可选），实际文件通过 API 访问
...
位于 `src/main/data/api/handlers/nodes.ts`，合并到 `apiHandlers`。
...
```typescript
// 获取某目录下的子节点（文件树懒加载）
const { data: children } = useQuery('/nodes/:id/children', {
  params: { id: folderId },
})
...
// 移入 Trash
const { trigger: trashNode } = useMutation('PUT', '/nodes/:id/trash', {
  refresh: ['/files/nodes'],
})
await trashNode({ params: { id: nodeId } })

// 恢复
const { trigger: restore } = useMutation('PUT', '/nodes/:id/restore', {
  refresh: ['/files/nodes'],
})

// 查询文件被谁引用
const { data: refs } = useQuery('/nodes/:id/refs', {
...
上传是原子操作：物理文件写入 + 节点创建在同一个 main 进程事务中完成，解决了 RFC 识别的 P1/P2 问题。
...
| 文件路径 | 内容 |
|---------|------|
| `src/main/data/db/schemas/node.ts` | `nodeTable` + `fileRefTable` Drizzle Schema（第三章定稿） |
| `packages/shared/data/types/fileNode.ts` | `FileNode`、`FileRef`、DTO 类型定义（第六章） |
| `packages/shared/data/types/fileProvider.ts` | Provider Config Zod Schema（第二章） |
| `packages/shared/data/api/schemas/files.ts` | `FileSchemas` API 类型声明（第六章） |
...
| `src/main/data/api/handlers/files.ts` | DataApi handlers（第六章） |
...
1. `FileNodeService`：
   ...
   - `trash()` / `restore()` 实现 OS 风格 Trash 逻辑（第四章）
...
### 7.5 Phase 4: Consumer Migration（消费方迁移）
...
| 新旧系统并行期数据冲突 | Phase 4 的 `toFileMetadata` 适配层保证旧消费方仍可工作 |
